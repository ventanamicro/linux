...
/* Control register common accross all RISC-V trace components */
...
/* Implementation register common accross all RISC-V trace components */
 ...
 * shared accross the RISC-V trace drivers to infer trace parameters,
...
extern const struct rvtrace_component_id *rvtrace_match_id(struct rvtrace_component *comp,
							   const struct rvtrace_component_id *ids);
extern struct rvtrace_component *rvtrace_find_by_fwnode(struct fwnode_handle *);

extern int rvtrace_poll_bit(struct rvtrace_platform_data *pdata, int offset,
...
extern int rvtrace_path_start(struct rvtrace_path *path);
extern int rvtrace_path_stop(struct rvtrace_path *path);
...
extern int rvtrace_path_copyto_auxbuf(struct rvtrace_path *path,
...
extern int __rvtrace_register_driver(struct module *owner, struct rvtrace_driver *rtdrv);
...
extern int rvtrace_perf_init(void);
extern void rvtrace_perf_exit(void);
